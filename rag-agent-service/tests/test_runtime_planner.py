...
def state(task: str, *, budget: RuntimeBudget | None = None) -> dict:
    ...
    return {
        ...
        "agent_snapshot": {
            ...
            "spec": {
                "knowledge": [
                    {"knowledge_base": "quality-manual", "version": "2026-01"}
                ],
...
def test_llm_call_limit_is_enforced_outside_the_decision_engine() -> None:
    ...
    engine = LlmEngine(
        [AgentDecision(action=AgentAction.ANSWER, final_answer="unsafe")]
    )
    graph = AgentGraph(
        engine,
...
def test_tool_approval_interrupt_survives_process_restart(tmp_path) -> None:
    ...
    resumed = second_graph.resume(
        "durable-thread",
        ApprovalResume(
            approved=True, approval_id="approval-001", decided_by="reviewer"
        ),
        max_steps=5,
    )
    ...
    tool_observation = next(
        item for item in resumed.observations if item["type"] == "tool"
    )
