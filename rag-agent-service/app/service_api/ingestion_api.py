...
def upload_document(
    ...
    document = container.repository.save_document(
        Document(
            ...
            file_path=path,
            sha256=sha256,
            metadata={"tenant_id": x_tenant_id, "uploaded_by": x_user_id},
        )
    )
    job = container.job_store.create(IngestionJob(job_type="PARSE", document_id=document.document_id))
...
def create_job(payload: JobCreateRequest, request: Request) -> IngestionJob:
...
def get_job(job_id: str, request: Request) -> IngestionJob:
    job = request.app.state.container.job_store.get(job_id)
    if job is None:
