...
def infer_profile(task: str, intent: str, metadata: dict[str, Any]) -> ProfileDecision:
    ...
    if any(
        word in lowered
        for word in ("no search", "without retrieval", "不要检索", "仅根据上文")
    ):
        return ProfileDecision(
    ...
    if any(
        word in lowered
        for word in ("deep research", "cross-check", "全面调研", "交叉验证")
    ):
        return ProfileDecision(
...
def resolve_profile(
    ...
    *,
    ...
    return EffectiveRetrievalPolicy(
        profile=selected, decision=outcome, reason=reason, **template
    )
